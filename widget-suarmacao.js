(function () {
    ...
    (function() {
        function injectPLBadge() {
            try {
                ...
                a.href = 'https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=suarmacao';
                a.target = '_blank';
                a.rel = 'noopener';
    ...
    const WEBHOOK_CHECK_LIMIT = 'https://n8n.segredosdodrop.com/webhook/suarmacao-check-limit';
    ...
    const styles = `
        ...
        :root {
            --c-bg: #ffffff;
            --c-surface: #f5f3ff;       /* Lilás muito claro (background da marca) */
            --c-ink: #2a2470;           /* Navy/purple escuro do logo ARMACAO */
            --c-muted: #8a87b8;
            --c-line: #e0dcf5;
            --c-accent: #6f4ce0;        /* Roxo médio da marca */
            --c-danger: #e94a5c;        /* Coral red do SUA */
        ...
        .q-input.is-error {
            ...
            background: rgba(233,74,92,0.06) !important;
            box-shadow: 0 0 0 3px rgba(233,74,92,0.15);
        }
        .q-face-frame.is-error {
            outline: 3px solid var(--c-danger);
            outline-offset: 2px;
            background: rgba(233,74,92,0.06);
        }
        .q-validation-hint {
            display: none;
            ...
            margin-bottom: 12px;
            text-align: center;
            box-shadow: 0 3px 10px rgba(233,74,92,0.25);
            ...
            <div class="q-card-ia">
                ...
                <div class="q-content-scroll">
                    ...
                    <div id="q-header-provador">
                        ...
                        <img src="https://acdn-us.mitiendanube.com/stores/001/669/128/themes/common/logo-418168872-1686683314-cd3d2ab59e161b7fe9918a9282d850611686683314-480-0.png" alt="SUA ARMACAO" style="height:48px;width:auto;"/>
                    </div>
                    ...
                    <div id="q-loading-box">
                        <div class="q-loading-texts">
                            ...
                            <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=suarmacao" target="_blank" class="q-loading-t2">
                ...
                <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=suarmacao" target="_blank" class="q-powered-footer">
                    <span>Powered by</span>
    ...
    function init() {
        ...
        function checkFields() {
            // Botão sempre clicável — a validação acontece no onclick.
            // Quando tudo está OK, limpa o hint que pode estar aberto.
            const nums = phoneInput.value.replace(/\D/g, '');
            const phoneOk = isValidBRPhone(nums);
            const allOk = userPhoto && phoneOk && document.getElementById('q-accept-terms').checked;
            if (allOk) {
                const hint = document.getElementById('q-validation-hint');
                if (hint) hint.classList.remove('is-visible');
                phoneInput.classList.remove('is-error');
                const faceFrame = document.getElementById('q-face-frame');
...
const fd = new FormData();
        ...
        function flashError(targetEl, hintMsg) {
            ...
            if (targetEl) {
                ...
                setTimeout(() => targetEl.classList.remove('q-shake'), 600);
        ...
        genBtn.onclick = async () => {
            ...
            // Valida em ordem: WhatsApp → foto → termos
            if (!_gPhoneOk) {
                flashError(phoneInput, 'Preencha seu WhatsApp para continuar');
                return;
            }
            if (!userPhoto) {
                flashError(faceFrame, 'Envie ou tire sua foto para continuar');
                return;
            }
            if (!document.getElementById('q-accept-terms').checked) {
                const termsRow = document.querySelector('.q-terms-row');
                flashError(termsRow, 'Aceite os termos para continuar');
                return;
            }

            // tudo ok — esconde hint, libera
            const hint = document.getElementById('q-validation-hint');
            if (hint) hint.classList.remove('is-visible');
            phoneInput.classList.remove('is-error');
            if (faceFrame) faceFrame.classList.remove('is-error');
        ...
        // Tira o erro do campo assim que o cliente começa a corrigir
        phoneInput.addEventListener('input', () => {
            phoneInput.classList.remove('is-error');
            const hint = document.getElementById('q-validation-hint');
            if (hint) hint.classList.remove('is-visible');
